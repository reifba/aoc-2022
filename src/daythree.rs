...
const PATH_3: &str = "/Users/Barak/Documents/aoc/input3.txt";
...
pub fn get_item_from_split(s: &str) -> Option<char> {
    let length=s.len() / 2;
...
fn multiple_interscetion(v: &[HashSet<char>]) -> char {
    ...
    v[0].iter().filter(|c| v[1].contains(c) && v[2].contains(c)).cloned().next().unwrap()
